...
namespace DocumentosOrtobio
{
    public partial class SettingsForm : Form
    {
        ...
        private void BtnManageDocuments_Click(object sender, EventArgs e)
        {
            ...
            {
                var docManagementForm = new DocumentManagementForm(basePath, loggedInUser, categoriesWithSubmenus);
        ...
        {
            string logFilePath = Path.Combine(basePath, "activity_log.txt");
            string logMessage = $"{DateTime.Now:dd-MM-yyyy HH:mm:ss} - {GetLocalIPAddress()} - {loggedInUser} - {activity}{Environment.NewLine}";
